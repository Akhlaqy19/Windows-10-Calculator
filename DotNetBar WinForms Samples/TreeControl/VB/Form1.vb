...
Public Class Form1
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ' For email tree we will override sorting behavior
        ' so sort is performed on actual email nodes which are at 2nd level
        AddHandler advTree2.Columns.SortCells, AddressOf EmailSortCells
    ...
    ' For email tree we override default sorting behavior which sorts root nodes only
    ' We will cancel built-in sort and sort 2nd level of nodes which are actual email messages
    ' So when Subject column header is clicked actual email headers are sorted while parent nodes are left untouched
    Private Sub EmailSortCells(ByVal sender As Object, ByVal e As SortEventArgs)
        e.Cancel = True ' Cancel built-in sorting
        For Each node1 As Node In advTree2.Nodes
            For Each node2 As Node In node1.Nodes
                node2.Nodes.Sort(0, node2.Nodes.Count, e.Comparer)
            Next node2
        Next node1
    End Sub
